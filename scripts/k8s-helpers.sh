...
check_cluster() {
  ...
  which kubectl
  cluster=$(kubectl -n $NAMESPACE config current-context)
...
get_newest_pod_by_app_name() {
  APP_NAME=$1
  kubectl -n $NAMESPACE get pod -l app="$APP_NAME" -o jsonpath="{.items[0].metadata.name}" --sort-by=.status.startTime | tail -n 1
}
